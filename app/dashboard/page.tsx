...
export default function Dashboard() {
  ...
  const questionData = Object.entries(data.questionAverages || {})
    .filter(([key]) => key !== 'q8') // exclude q8 from averages
    .map(([name, value]) => ({
      name: `Q${name.slice(1)}`,
  ...
  const graduationYearData = Object.entries(
    data.rawData?.reduce((acc, curr) => {
      if (curr.graduation_year) {
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold">{data.totalResponses || 0}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h2 className="text-xl font-semibold mb-2">Average Impact Score</h2>
          <p className="text-3xl font-bold">{(data.averageScore || 0).toFixed(2)}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h2 className="text-xl font-semibold mb-2">Most Common Evaluation</h2>
          <p className="text-3xl font-bold">
            {Object.entries(data.evaluationCounts || {}).reduce((a, b) => a[1] > b[1] ? a : b, ['', 0])[0] || 'N/A'}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        ...
        <div className="space-y-4">
          {data.rawData?.filter(response => response.q8).map((response, index) => (
